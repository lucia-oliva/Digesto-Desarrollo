        ...
        if (!cancelled) {
          setError(
            requestError?.response?.data?.msg ||
              "No se pudieron cargar las normativas más consultadas.",
          );
        }
      } finally {
        if (!cancelled) {
        ...
        <div className="hero-content text-center flex items-start py-24">
          <div className="w-full lg:w-2/3 px-2 sm:px-4 mx-auto">
            ...
            <p className="mb-4 text-xl font-sans font-light text-base-100">
              La plataforma que sirve como espacio digital para
              consultar las normativas y toda documentación emitida
              por todas las dependencias de la Universidad Nacional
              de La Rioja.
            </p>
            ...
            >
              Búsqueda avanzada
              <IoMdSearch size={20} className="ml-2" />
            </a>
          </div>
        </div>
      </div>
