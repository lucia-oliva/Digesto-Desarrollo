...
export function PdfViewer({ filename, pdfUrl, setPdfUrl }) {
  ...
  useEffect(() => {
    ...
    const cargarPdf = async () => {
      ...
      try {
        ...
        if (!cancelled) {
          setError(
            "No se pudo cargar el documento. Intente nuevamente.",
          );
        }
      } finally {
        if (!cancelled) {
   ...
   */
  const effectivePdfUrl = blobUrl || pdfUrl;
  const showLoading = loading && !effectivePdfUrl;
