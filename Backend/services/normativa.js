...
function httpError(status, internalMessage) {
...
async function getNormativaCompletaById(id) {
  ...
  const tags = tagRows.map((row) => row.nombre);
...
async function updateModificacion({ id_relacion, accion, comentario }) {
  ...
  return {
    message: `Relación ${id_relacion} actualizada`,
  };
}
...
async function deleteModificacion(id) {
  const result = await db.execute("DELETE FROM relacion WHERE id = ?", [id]);
...
async function registrarModificacion({
  ...
  return {
    ...
    message: `Modificación registrada (${result.insertId})`,
  };
}
...
async function eliminar(id, userId, motivo = null) {
  ...
  if (!row) {
    throw httpError(404, `Normativa ${id} no encontrada`);
  }

  if (row.estado === "eliminada") {
    return {
      message: "La normativa ya estaba eliminada",
    };
  }
